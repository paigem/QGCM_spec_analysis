...
def main(datapath,dataname1,dataname2,terms_dict):
	...
	### Load QGCM data
	wek_data = Dataset(datapath+dataname2)
	wek = wek_data.variables['wekt']
	del wek_data
	wek = np.transpose(np.squeeze(wek,(2,1,0))) # dimensions: (x,y,time)

	if terms_dict.get('print_stuff'):
		print 'wek.shape=',wek.shape
		print 'Mem usage before transfer func =',resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1000

	# Average over p's to interpolate onto T-grid and match wekt
	p = avg_dim(p,'xy',1)
	...
	if terms_dict.get('spatial_flag'):
		transfer_iso,kiso,ktiso = calc_T_func.main(p,wek,terms_dict)
		del kiso
	else:
		transfer_iso,kiso,ktiso = calc_T_func.main(p,wek,terms_dict)
	del p,wek
	...
	if terms_dict.get('save_data'):
		...
		if terms_dict.get('spatial_flag'):
			#Tgrp = Dataset('/g/data/v45/pm2987/netcdf_transfers/windstress_spatial_1yr_test_layer1_yr159_dg2_output037.nc', 'w', format='NETCDF3_CLASSIC')
			Tgrp = Dataset('/g/data/v45/pm2987/netcdf_transfers/windstress_spatial'+save_name+extra_name+'_'+str(x1)+'_'+str(x2)+'_'+str(y1)+'_'+str(y2)+'_'+str(yrs[0])+'_'+str(yrs[1])+'.nc', 'w', format='NETCDF3_CLASSIC')
			...
			#Tgrp = Dataset('/g/data/v45/pm2987/netcdf_transfers/windstress_1yr_test_layer1_yr159_dg2_output037.nc', 'w', format='NETCDF3_CLASSIC')
			Tgrp = Dataset('/g/data/v45/pm2987/netcdf_transfers/windstress_'+save_name+extra_name+'_'+str(x1)+'_'+str(x2)+'_'+str(y1)+'_'+str(y2)+'_'+str(yrs[0])+'_'+str(yrs[1])+'.nc', 'w', format='NETCDF3_CLASSIC')
	...
	if terms_dict.get('spatial_flag'):
		...
		plt.xscale('log')
		plt.axis('tight')
		plt.title('Windstress term in energy budget, integrated over wavenumber')
		...
		plt.xscale('log')
		plt.axis('tight')
		plt.title('Windstress term in energy budget, integrated over frequency')
		...
		plt.xscale('log')
		plt.axis('tight')
		plt.title('Windstress term in energy budget, averaged over space')
		plt.xlabel('Frequency')
		plt.ylabel('(nW/kg)/(rad/day)')
		plt.legend()
